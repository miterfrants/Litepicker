...
export class Calendar {
    ...
    protected render() {
        ...
        if (style[`columns${this.options.numberOfColumns}`]) {
            ...
            months.classList.add(
                style[`columns${this.options.numberOfColumns}`]
            );
        }
    ...
    protected renderMonth(date: DateTime) {
        ...
        if (this.options.dropdowns.years) {
            ...
            selectYears.addEventListener("change", (e) => {
                ...
                if (typeof this.options.onChangeYear === "function") {
                    this.options.onChangeYear.call(
                        this,
                        this.calendars[idx],
                        idx
                    );
            ...
            monthYear.innerHTML = String(
                date.getFullYear() +
                    (this.options.fullYearText
                        ? this.options.fullYearText
                        : "") +
                    " "
            );
        ...
        if (this.options.dropdowns.months) {
            ...
            for (let x = 0; x < 12; x += 1) {
                ...
                option.text = optionMonth.toLocaleString(this.options.lang, {
                    month: "long",
                });
                option.disabled =
                    (this.options.minDate &&
            ...
            monthName.innerHTML = date.toLocaleString(this.options.lang, {
                month: "long",
            });
        ...
        for (let idx = 1; idx <= totalDays; idx += 1) {
            ...
            if (
                ...
                startDate.getDay() === this.options.firstDay
            ) {
                days.appendChild(this.renderWeekNumber(startDate));
            }
    ...
    protected renderDay(date: DateTime) {
        ...
        if (this.datePicked.length) {
            ...
            if (
                ...
                this.options.endDate.toDateString() === date.toDateString()
            ) {
                day.classList.add(style.isEndDate);
            }
            ...
            if (this.options.startDate && this.options.endDate) {
                if (
                    date.isBetween(this.options.startDate, this.options.endDate)
                ) {
                    day.classList.add(style.isInRange);
                }
            }
        ...
        if (
            this.options.minDate &&
            date.isBefore(new DateTime(this.options.minDate))
        ) {
        ...
        if (
            this.options.maxDate &&
            date.isAfter(new DateTime(this.options.maxDate))
        ) {
            day.classList.add(style.isLocked);
        }

        if (this.options.minDays && this.datePicked.length === 1) {
            const left = this.datePicked[0]
                .clone()
                .subtract(this.options.minDays, "day");
            const right = this.datePicked[0]
                .clone()
                .add(this.options.minDays, "day");
        ...
        if (this.options.lockDays.length) {
            const locked = this.options.lockDays.filter((d) => {
                if (d instanceof Array) {
                    return date.isBetween(
                        d[0],
                        d[1],
                        this.options.lockDaysInclusivity
                    );
                }
        ...
        if (this.options.highlightedDays.length) {
            const isHighlighted = this.options.highlightedDays.filter((d) => {
                if (d instanceof Array) {
      ...
      `;
        ...
        if (this.options.singleMode) {
            if (this.datePicked.length === 1) {
                const startValue = this.datePicked[0].format(
                    this.options.format,
                    this.options.lang
                );
                footer.querySelector(
                    `.${style.previewDateRange}`
                ).innerHTML = startValue;
            }
        } else {
            if (this.datePicked.length === 1) {
                footer
                    .querySelector(`.${style.buttonApply}`)
            ...
            if (this.datePicked.length === 2) {
                ...
                const endValue = this.datePicked[1].format(
                    this.options.format,
                    this.options.lang
                );
    ...
    protected dateIsBooked(date, inclusivity) {
        return this.options.bookedDays.filter((d) => {
            if (d instanceof Array) {
    ...
    private weekdayName(day, representation = "short") {
        ...
        if (this.options.replaceWeekdayString) {
            weekdayName = weekdayName.replace(
                this.options.replaceWeekdayString,
                ""
            );
        }
        return weekdayName;
